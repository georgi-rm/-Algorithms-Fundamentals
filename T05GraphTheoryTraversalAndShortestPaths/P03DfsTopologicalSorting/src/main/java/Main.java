...
public class Main {
    ...
    public static Collection<String> topSort(Map<String, List<String>> graph) {
        List<String> topologicallySorted = new ArrayList<>();

        Set<String> visited = new HashSet<>();
        Set<String> cycles = new HashSet<>();

        for (String node : graph.keySet()) {
            topSortDfs(topologicallySorted, visited, cycles, node, graph);
        }

        return topologicallySorted;
    }

    private static void topSortDfs(List<String> topologicallySorted, Set<String> visited, Set<String> cycles, String node, Map<String, List<String>> graph) {
        ...
        for (String childNode : graph.get(node)) {
            topSortDfs(topologicallySorted, visited, cycles, childNode, graph);
        }
        cycles.remove(node);
